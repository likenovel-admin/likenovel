...
- deep monitor는 `--scheduled` 자동 보강 기준으로 경고하되 전체 readiness·결손 정보와 `automaticPolicyCounts`의 대상 밖 잔여 결손을 분리 보존한다. 선택된 캐릭터의 RP 필수 내용 누락, 주인공 미식별, identity 오류는 계속 경고한다. 기존 RP 삭제, consumer exact-key gate 완화, 미동의 작품 수집은 하지 않는다. 변경 배포 시 backend audit CLI를 먼저 배포한 뒤 해당 옵션을 쓰는 monitor를 반영한다.
