...
- 정기 RP/캐릭터별 scene 보강 대상은 앞 30개 공개 회차의 signal 범위와 기존 `build_inventory_rp_targets` 선택을 함께 적용한다(설정 기본 최대 2명). 첫 30개 회차 자체의 scene 완비 검사는 별도로 유지한다. 배치는 읽기 전용 audit의 `--scheduled-action-ids`가 반환한 버전·양수 ID 검증을 통과한 보강/검수 작품 목록을 후보 SQL의 정렬·LIMIT 전에 반영한다. 판정 실패를 빈 후보로 처리하지 않는다.
- deep monitor는 `--scheduled` 자동 보강 기준으로 경고하되 전체 readiness·결손 정보와 `automaticPolicyCounts`의 대상 밖 잔여 결손을 분리 보존한다. Foundation 보강은 첫 30개 공개 회차의 활성 요약·캐릭터 신호 누락을 직접 확인한다. 전체 작품 기준 `processing`만으로 보강 대상으로 잡지 않으며, 명시적인 `failed` 상태는 계속 경고한다. 선택된 캐릭터의 RP 필수 내용 누락, 주인공 미식별, identity 오류는 계속 경고한다. 기존 RP 삭제, consumer exact-key gate 완화, 미동의 작품 수집은 하지 않는다. 변경 배포 시 backend audit CLI를 먼저 배포한 뒤 해당 옵션을 쓰는 monitor를 반영한다.
