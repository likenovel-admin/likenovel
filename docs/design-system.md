...
- 메인 캐릭터 구좌는 CMS의 `자동 모드`를 기본으로 사용한다. 자동 모드는 전체 목록의 추천순 상위 24명 중 전용 캐릭터 이미지가 있는 후보를 먼저 무작위로 최대 12명 선택하고, 인원이 모자랄 때만 나머지 후보로 채운다. `수동 선택 모드`에서는 기존 CMS 편성의 `card_order` 상위 12명을 사용한다. 데스크톱은 6열 2행으로 전체를 노출해 페이지 이동을 숨기고, 태블릿은 4열 2행, 모바일은 2열 2행 단위로 좌우 순환한다.
...
- CMS 캐릭터챗 노출 관리는 `자동 모드`와 `수동 선택 모드`, 그리고 `메인 12명 편성`, `전체 공개`, `후보 검수`를 분리한다. 자동 모드에서도 수동 편성은 전환 시 사용할 예비 편성으로 보존한다. 후보 검수에는 캐릭터별 회차 수, RP 예시 수, 장면 수와 품질 사유를 표시하고 품질 미달 캐릭터의 등록·수정은 서버와 UI 양쪽에서 막는다.
