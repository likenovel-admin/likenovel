...
- 홈 캐릭터 카드 진입은 채팅 화면을 먼저 연 뒤 세션을 생성한다.
...
- 메인 캐릭터 구좌의 CMS 작품 후보는 공개 회차가 10화 이상이고 플랫폼 내 AI 콘텐츠 서비스에 동의한 공개 작품으로 제한한다. 해당 작품의 `character_inventory_v3`에서 공개 채팅 적격과 표시 안전 조건을 통과한 주·조연을 관리자가 선택한다.
...
- 선택 주인공 대사는 CMS 캐릭터 이미지 스냅샷을 사용한다. 화자가 명시된 다른 인물 대사는 기본 원형 프로필을 사용하며, 캐릭터명은 모든 대사 말풍선 위에 표시한다.
