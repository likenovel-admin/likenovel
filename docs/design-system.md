...
- 전체 목록 상단은 `추천순 / 전체 / 읽고 있는 작품 / 처음 보는 작품` 4개 칩을 사용하고 `추천순`을 기본값으로 한다. 추천순은 현재 공개 회차 중 유효한 채팅 회차요약의 완성 비율을 사용해 완전 준비 캐릭터를 먼저 두고, 비율이 같으면 CMS 원순서를 유지한다. 작품의 절대 회차 수는 추천 가중치로 사용하지 않는다. `읽고 있는 작품`은 최근 열람 시각 내림차순으로 정렬하며, 비로그인 사용자는 추천순과 전체를 그대로 이용하고 개인화 필터 선택 시 현재 filter query를 보존한 로그인 흐름으로 보낸다.
- 홈 12명과 전체 목록은 같은 서버 공개 조건을 사용한다. 공개 채팅·구좌 적격 캐릭터 프로필, 첫 등록 회차 범위에서 쓸 수 있는 RP 예시(표시 제목이 `0화 프롤로그`인 경우 포함), 현재 첫 등록 회차의 회차요약, 정확한 캐릭터 키와 장면 요약이 포함된 첫 등록 회차 장면 증거, 원문 문서·청크가 모두 있어야 하며, 미달 행은 CMS에 보존되어도 유저웹 응답에서는 자동 제외된다.
