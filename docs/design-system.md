...
- 주인공챗 선택 카드에는 이미지, 작품명, 캐릭터명, `~N화까지`만 표시한다. `양호`, `보통`, `부족`, readiness, 점수, 퍼센트 같은 CMS 운영 상태는 유저웹에 노출하지 않는다.
