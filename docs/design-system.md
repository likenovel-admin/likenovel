...
- 전체 목록 상단은 `전체 / 읽고 있는 작품 / 처음 보는 작품` 3개 칩만 사용한다. `전체`와 `처음 보는 작품`은 CMS 원순서를 유지하고, `읽고 있는 작품`은 최근 열람 시각 내림차순으로 정렬하며 비로그인 선택은 현재 필터 query를 보존한 로그인 흐름으로 보낸다.
- 홈 12명과 전체 목록은 같은 서버 공개 조건을 사용한다. 캐릭터 프로필, RP 예시, 정확한 캐릭터 키가 포함된 장면 증거가 모두 있어야 하며, 미달 행은 CMS에 보존되어도 유저웹 응답에서는 자동 제외된다.
