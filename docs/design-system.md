...
- 캐릭터챗 세션은 RP 전용이므로 입력창에서 모드 선택기를 노출하지 않는다. 같은 위치에는 요술봉 선택지 버튼을 두고, 켜진 동안 최신 캐릭터 답변 뒤에 사용자 선택지 3개를 표시한다. 일반 웹소챗의 모드 선택 UI는 유지하되 `인물과 대화`는 노출하지 않고, 메인 캐릭터 구좌 같은 전용 진입과 자연어 요청으로만 RP를 시작한다.
- 메인 캐릭터 구좌의 CMS 작품 후보는 공개 회차가 15화 이상이고 플랫폼 내 AI 콘텐츠 서비스에 동의한 공개 작품으로 제한한다. 해당 작품의 `character_inventory_v3`에서 공개 채팅 적격과 표시 안전 조건을 통과한 주·조연을 관리자가 선택한다.
- 메인 캐릭터 구좌는 총 12개를 운영한다. 데스크톱은 6열 2행으로 전체를 노출해 페이지 이동을 숨기고, 태블릿은 4열 2행, 모바일은 2열 2행 단위로 좌우 순환한다.
