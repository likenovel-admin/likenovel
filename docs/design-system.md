...
- 홈 구좌 우상단의 기존 `더보기`로만 `/product/character-chat` 전체 목록에 진입한다. 홈에서 진입한 기본 추천 화면은 방금 홈 응답으로 노출한 최대 12명을 같은 순서로 먼저 유지하고, 전체 목록의 나머지를 캐릭터 ID 중복 없이 이어 붙인다. 범위·역할·정렬을 변경하거나 필터를 초기화하면 홈 고정 순서를 해제한다. 별도 GNB·메뉴 진입점은 추가하지 않는다.
