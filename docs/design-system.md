...
- 홈은 CMS에서 현재 활성화한 구좌의 `card_order` 상위 12명을 노출하고, `더보기` 전체 목록은 공개·비블라인드·AI 콘텐츠 서비스 동의·공개 15화 이상 작품 중 `public_chat_eligible` 캐릭터에 유효한 RP 프로필, 어느 회차든 비어 있지 않은 RP 예시, 활성 회차요약, 캐릭터 키가 일치하는 사용 가능 장면이 있는 행을 자동 후보로 삼는다. 후보는 작품·canonical character key 기준으로 중복 제거하고 작품당 최대 2명까지 주인공 우선으로 노출하며, 이미지는 캐릭터 이미지 → 작품 표지 → 기본 이미지 순으로 대체한다. 전체 목록에는 수집 기준일·연재상태·`public_slot_eligible`·첫 회차의 정확한 캐릭터 장면 일치·RP 예시 0~1화 제한을 hard gate로 사용하지 않는다. 세션 장면은 선택한 캐릭터와 정확히 일치하는 읽은 범위 내 장면을 우선하고, 없으면 같은 읽은 범위의 실제 작품 장면에 IF 방식으로 합류하되 미래 회차는 사용하지 않는다.
