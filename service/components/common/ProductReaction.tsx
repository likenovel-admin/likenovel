...
const ProductReaction = ({ evaluations, emojiPosition = "inside" }: Props) => {
  ...
  return (
    <div
      className={`flex flex-col gap-6pxr max-w-[741px] ${
    ...
    >
      {ratings?.map(([rating, count], index) => {
        ...
        return (
          <div
            key={rating}
            className="flex items-center justify-between bg-light-gray-100 rounded-full"
          >
            <div className="relative flex items-center w-full">
              <div
                className={`${getColorClass(
                  percentage
                )} h-[28px] rounded-full transition-all duration-300`}
                style={{ width: `${percentage}%` }}
              />
              <Image
                src={emojis[index]}
                width={emojiPosition === "inside" ? 20 : 24}
                height={emojiPosition === "inside" ? 20 : 24}
                alt={rating}
                className={`absolute ${
                  emojiPosition === "inside"
                    ? "left-2 h-[20px] w-[20px]"
                    : "left-[-29px] h-[24px] w-[24px]"
                }`}
              />
              <span
                className={`absolute text-13pxr font-medium px-3 ${
                  emojiPosition === "inside" ? "left-[22px] text-[#373B46]" : ""
                }`}
              >
                {messages[index]}
              </span>
