...
/**
 * Route Handler to save NICE find-id session data
 * Route Handlers can modify cookies in Next.js App Router
 */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(
      `[${new Date().toISOString()}] Saving NICE find-id session data:`,
      { redirectUrl, logData }
    );

    const session = await getIronSession(request, res, getSessionOptions());

    (session as any).findIdResult = redirectUrl;
