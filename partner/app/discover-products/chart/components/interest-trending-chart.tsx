...
import { InterestTrend } from "@/types/product-discovery-statistics";
...
const InterestTrendingChart = ({ data }: { data: InterestTrend[] }) => {
  const isMoblie = useIsMobile();
  const formatData = React.useMemo(() => {
    return data.map((d) => {
      return {
        month: d.date ? format(d.date, "MM.dd") : "",
        desktop: d.count_interest_sustain,
        mobile: d.count_interest_loss,
      };
    });
  }, [data]);

  const sumInterestSustain = React.useMemo(() => {
    return data.reduce((acc, cur) => acc + cur.count_interest_sustain, 0);
  }, [data]);
  const sumInterestLoss = React.useMemo(() => {
    return data.reduce((acc, cur) => acc + cur.count_interest_loss, 0);
  }, [data]);
  return (
    <Card>
      <CardHeader>
        <CardTitle>관심 유지수 추이</CardTitle>
...
export default InterestTrendingChart;
