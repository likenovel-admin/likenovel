...
export default function DistributionWorksTable({ data, loading }: Props) {
  ...
  const goToProductView = (productId: string) => {
    router.push(`/products/distribution/upload?mode=view&id=${productId}`);
  };

  const goToProductEdit = (productId: string) => {
    router.push(`/products/distribution/upload?mode=edit&id=${productId}`);
  };
  ...
  const columns: Column[] = [
    ...
    { header: "작가명", key: "author_nickname" },
    { header: "회차수", key: "count_episode" },
    { header: "계약 유형", key: "contract_type" },
    { header: "해당CP", key: "cp_company_name" },
    {
      header: "작품 등록일",
      key: "created_date",
      render: (_, row: IProduct) =>
    ...
    { header: "1차 장르", key: "primary_genre" },
    { header: "2차 장르", key: "sub_genre" },
    { header: "판매 상태", key: "price_type" },
    {
      header: "독점 여부",
      key: "monopoly_yn",
