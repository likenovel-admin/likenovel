...
import {
  ...
  IGetEpisodeApplyListParams,
  IGetEpisodeApplyListResponse,
...
export const useGetEpisodeApplyList = (
  params: IGetEpisodeApplyListParams,
  enabled = true
) => {
  return useQuery<IGetEpisodeApplyListResponse>({
    queryKey: ["GetEpisodeApplyList", JSON.stringify(params)],
    queryFn: async () => {
      const res = await apiClient.request<IGetEpisodeApplyListResponse>({
        url: "/v1/query/admins/apply-episode",
        method: "GET",
        queryParams: params,
      });
      return res;
...
export const useAcceptEpisodeApply = () => {
  return useMutation<IProcessEpisodeApplyResponse, Error, string>({
    mutationFn: async (id: string) => {
      return await apiClient.request<IProcessEpisodeApplyResponse>({
        url: `/v1/command/admins/apply-episode/${id}/accept`,
        method: "POST",
      });
    },
  });
...
export const useDenyEpisodeApply = () => {
  return useMutation<IProcessEpisodeApplyResponse, Error, string>({
    mutationFn: async (id: string) => {
      return await apiClient.request<IProcessEpisodeApplyResponse>({
        url: `/v1/command/admins/apply-episode/${id}/deny`,
        method: "POST",
      });
    },
  });
...
export const useStartEpisodeSale = () => {
  return useMutation<IEpisodeSaleResponse, Error, IEpisodeSaleStartRequest>({
    mutationFn: async (body: IEpisodeSaleStartRequest) => {
      return await apiClient.request<IEpisodeSaleResponse>({
