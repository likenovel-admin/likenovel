...
export const ImageViewBlock: React.FC<NodeViewProps> = ({
  ...
  const containerMaxWidth = React.useMemo(() => {
    ...
    // Fallback to actual container width
    return containerRef.current.offsetWidth || Infinity
  ...
  const effectiveMaxWidth = React.useMemo(() => {
    return Math.min(maxWidth, containerMaxWidth)
  ...
  return (
    <NodeViewWrapper
      ref={containerRef}
      data-drag-handle
      className="relative leading-none max-w-full"
    >
      <div
        className={cn("group/node-image relative rounded-md object-contain max-w-full", alignmentClass)}
        style={{
          maxWidth: effectiveMaxWidth > 0 ? `${effectiveMaxWidth}px` : "100%",
          width: Math.min(currentWidth, effectiveMaxWidth),
          ...
          <div className="h-full contain-paint">
            <div className="relative h-full">
              ...
              >
                <img
                  className={cn(
                    "h-auto rounded object-contain transition-shadow max-w-full",
                    {
